...
describe('real districting', () => {
  ...
  test('reports the valid flag from contiguity and deviation', () => {
    // The synthetic 4-unit grid splits into two contiguous, equal-population
    // districts, so county integrity yields a valid plan.
    const result = districtRealByCountyIntegrity(testDataset, {
      numDistricts: 2,
      seed: 1,
    });
    expect(result.metrics.contiguousDistricts).toBe(2);
    expect(result.metrics.maxDeviationFraction).toBe(0);
